function camExport() {
  ...
  for (i = 3; i <= count; i++) {
    ...
    if (user.isTicketCamChecked) { //ここを変更
      ...
      // 変える
      kashidashiDate = Utilities.formatDate(user.camDatetime, 'JST', 'yyyy年MM月dd日');
      ...
      body.replaceText('{rentalId}', user.camRentalId);
